...
@HiltViewModel
class MyRentViewModel @Inject constructor(
    ...
    fun fetchRentProducts() {
        _state.value = ResponseState.Loading

        viewModelScope.launch {
            val result = withContext(Dispatchers.IO) {
                orderRepository.fetchRentedProducts()
            }

            _state.value = result.fold(
                onSuccess = { response ->
                    val rentProducts = mutableListOf<Product>()
                    response.forEach { model ->
                        launch(Dispatchers.IO) {
                            ...
                            product.onSuccess {
                                rentProducts.add(
                                    it.copy(datePosted = model.rentDate)
                                )
                            }
                        }
                    }
                    ResponseState.Success(rentProducts)
                },
